...
from rl2020.activity import activity_context

__author__ = 'bkurniawan'

class MultiObjectiveQLearningTracesAgent(SeedableAgent):
    ...
    def __init__(self, num_actions, discretizer, reward_builder, seed=None) -> None:
        ...
        self.agents = [QLearningTracesAgent(num_actions, discretizer, seed) for _ in range(self.num_rewards)]
    ...
    def episode_start(self, activity_context: ActivityContext) -> None:
        ...
        self.current_epsilon = self.agents[0].current_epsilon # all agents will have identical current_epsilon, which is needed in select_action
    ...
    def select_action(self, state) -> int:
        if self.np_random.random() < self.current_epsilon:
            ...
            q_values_list = [agent.q[discrete_state] for agent in self.agents] # list of x ndarrays where x = num_rewards
    ...
    def save_policy(self, path): # used to save a policy that can be used for activity
        for i in range(self.num_rewards):
            path_i = path + '_' + str(i)
            file = open(path_i, 'wb')
            pickle.dump(self.agents[i].q, file)
            file.close()
