...
if __name__ == '__main__':
    ...
    num_rewards = 3
    results_name = 'dqn-momnn-random-' + str(num_learning_episodes) + '-' + rb + '-basic-a'
    #results_name = 'baseline-blue-stern-conversion-agent-basic-c2'
    out_path = 'rl2020_test_results/dqn-mo-mnn/' + results_name
    ...
    normalizer = AceZeroDQNNormalizer()
    input_dim = 4
    ...
    for i in range(start_trial, start_trial + num_trials):
        policy_path = 'rl2021_mo_results/dqn-mo-mnn/dqn-mo-mnn-random-20000-' + rb + '/policy0' + str(i) + '-' + str(num_learning_episodes) + '.p'
        print('policy path:', policy_path)
        tester_builder = MultiObjectiveMultiNNDQNTesterBuilder(policy_path, num_rewards=num_rewards, normalizer=normalizer, input_dim=input_dim, original_output_dim=original_output_dim)
